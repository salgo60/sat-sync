...
(function() {{
  ...
  async function loadOsmNotes() {{
    ...
    try {{
      ...
      const url = `https://api.openstreetmap.org/api/0.6/notes.json?bbox=${{bbox}}&limit=200&closed=0`;
      ...
      (data.features || []).forEach(f => {{
        ...
        m.bindPopup(`<div class="note-popup"><strong>💬 OSM Note</strong>${{escapeHtml(text)}}<br><small>${{date}}</small><br><a href="https://www.openstreetmap.org/note/${{f.properties.id}}" target="_blank">Öppna på OSM</a></div>`);
